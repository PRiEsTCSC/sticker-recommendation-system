...
use crate::{
    ...
    structs::database_structs::{
        Admin, DatabaseConnection, ManagementRequest, RegisterRequest, Session, User,
    },
};
...
impl DatabaseConnection {
    pub async fn new() -> Result<Self, sqlx::Error> {
        ...
        let redis =
            Arc::new(Client::open(load_redis_url()).expect("Failed to create Redis client"));
        Ok(Self { pool, redis })
    }
    ...
    pub async fn init_schema(&self) -> Result<(), sqlx::Error> {
        ...
        sqlx::query(
            r#"
            CREATE TABLE IF NOT EXISTS interaction_stickers (
                id UUID PRIMARY KEY,
    ...
    pub async fn save_interaction(
        ...
        sqlx::query(
            "INSERT INTO interactions (id, user_id, input_text, detected_emotion, sticker_data, created_at) VALUES ($1, $2, $3, $4, $5, $6)",
        )
        .bind(Uuid::new_v4())
        .bind(user_id)
        .bind(input_text)
    ...
    pub async fn save_sticker(
        &self,
        interaction_id: Uuid,
        sticker_data: &[u8], // Ensure parameter name matches
    ) -> Result<Uuid, sqlx::Error> {
        let sticker_id = Uuid::new_v4();
        // Use .execute() for INSERT and ignore the result (QueryResult)
        sqlx::query(
            "INSERT INTO interaction_stickers (id, interaction_id, sticker_data, created_at) 
            VALUES ($1, $2, $3, $4)",
        )
        .bind(sticker_id)
        .bind(interaction_id)
        .bind(sticker_data)
        .bind(chrono::Utc::now().naive_utc())
        .execute(&self.pool) // <-- Changed from fetch_one to execute
        .await?; // <-- Removed the map operation, just check for errors

        Ok(sticker_id) // <-- Return the generated ID
    }

    pub async fn update_sticker_metrics(
        &self,
        user_id: Uuid,
        sticker_data: &str,
    ) -> Result<(), sqlx::Error> {
        sqlx::query(
            r#"
            INSERT INTO sticker_metrics (id, user_id, sticker_data, usage_count, last_used)
            VALUES ($1, $2, $3, 1, $4)
            ON CONFLICT (user_id, sticker_data)
    ...
    //     let stickers = sqlx::query_scalar::<_, String>("SELECT sticker_data FROM sticker_metrics WHERE user_id = $1")
    //         .bind(user_id)
    //         .fetch_all(&self.pool)
    //         .await?;
    //     Ok(stickers)
    ...
    // pub async fn cache_sticker(&self, emotion: &str, sticker_data: &str) -> Result<(), redis::RedisError> {
    ...
    pub async fn cache_sticker(
        ...
        let value = serde_json::json!({
            ...
            "sticker_data": general_purpose::STANDARD.encode(sticker_data),
        })
        .to_string();
    ...
    //         let sticker_data = json["sticker_data"].as_str().unwrap_or_default().to_string();
    //         Ok(Some((emotion, sticker_data)))
    //     } else {
    //         Ok(None)
    //     }
    // }

    pub async fn get_cached_sticker(
        &self,
        key: &str,
    ) -> Result<Option<(String, Uuid, Vec<u8>)>, redis::RedisError> {
        let mut conn = self.redis.get_connection()?;
        if let Some(value) = conn.get::<_, Option<String>>(key)? {
            let json: serde_json::Value =
                serde_json::from_str(&value).expect("Failed to parse cached JSON");
            let emotion = json["detected_emotion"]
                .as_str()
                .unwrap_or("neutral")
                .to_string();
            let sticker_id = Uuid::parse_str(json["sticker_id"].as_str().unwrap_or_default())
                .unwrap_or(Uuid::new_v4());
            let sticker_data = general_purpose::STANDARD
                .decode(json["sticker_data"].as_str().unwrap_or_default())
                .unwrap_or_default();
            Ok(Some((emotion, sticker_id, sticker_data)))
        } else {
            Ok(None)
        }
    }
    ...
    pub async fn save_session(
        ...
        sqlx::query(
            "INSERT INTO sessions (id, user_id, admin_id, token, expires_at, created_at) VALUES ($1, $2, $3, $4, $5, $6)",
        )
        .bind(Uuid::new_v4())
        .bind(user_id)
        .bind(admin_id)
    ...
    pub async fn update_admin_login(
        &self,
        admin_id: Uuid,
        success: bool,
    ) -> Result<(), sqlx::Error> {
    ...
    pub async fn register_user(&self, req: RegisterRequest) -> Result<User, sqlx::Error> {
        let password_hash =
            bcrypt::hash(&req.password, bcrypt::DEFAULT_COST).expect("Failed to hash password");
    ...
    pub async fn update_user(
        ...
        if query_parts.is_empty() {
            log::warn!(
                "Attempted to update user {} with no provided fields (username or password).",
                user_id
            );
